...
export function useCalendar(jwtToken: string | null) {
  ...
  const fetchUserEvents = useCallback(async (jwtToken: string | null, fetchStart: number | null, fetchEnd: number | null) => {
    ...
    try {
      ...
      const parentCalendarPromises = parentCalendars.map(async (cal: any) => {
        ...
        return {
          normalCal: { id: cal.id, owner: cal.dataOwner, name: cal.summary, color: newCalendarObj.calendarDefaultColor, events: processedRaw },
          uniqueCal: { id: cal.id, owner: cal.dataOwner, name: cal.summary, color: newCalendarObj.calendarDefaultColor, events: recurringEvents }
        };
      });
  ...
  return { calendars, setCalendars, isLoading, error, uniqueCalendars, refetch: fetchUserEvents };
}